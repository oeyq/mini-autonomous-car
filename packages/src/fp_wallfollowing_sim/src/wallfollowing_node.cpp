...
private:
    ...
    struct Parameters {
        double desired_distance{0.7};    // Target distance from wall
        ...
        double max_angular_velocity{3.0}; // Maximum turning speed
    };
...
double AEBNode::calculateSpeed(double angular_velocity) const {
    ...
    if (abs_velocity > 0.2) return 1.5;      // Sharp turn
    if (abs_velocity > 0.1) return 2.5;      // Medium turn
    return 2.5;                              // Straight/gentle turn
}
